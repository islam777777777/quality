...
return;
                            ...
                            obj = {
                                ...
                                thinkess: thinkessGlobal,
                                thinkessPrivte :privateThinkess,
                                pointOne : one.value || '',
                                pointTwo : two.value,
                                ...
                                allDate: allDate || '',
                                diametermoasafa: Number(valuethinkess *1.009).toFixed(1) || '',
                        ...
                        dia.addEventListener("change",function(){
                            ...
                            specifications.addEventListener("change",function(){
                                ...
                                Array.from(document.getElementById("thivsDiam")).forEach((option) => {
                                    ...
                                    let baseValue = option.value.split(' * ')[0];
                                    option.value = baseValue;
                                    option.innerHTML = baseValue;
                                });

                                if (getDataChangeSpecifitions === 'DIN 8074/8075/2011-ES 1832/2008') {
                                    Array.from(document.getElementById("thivsDiam")).forEach((option) => {
                                        let value = parseFloat(option.value);
                                        finalThinkess = `${option.value} * ${(value / 16.86).toFixed(1)}`;
                                        option.innerHTML = finalThinkess;
                                        thinkessGlobal = Number(option.value);   
                                        console.log(value)
                                        privateThinkess  = option.innerHTML;
                                    });
                                }
                                else if (getDataChangeSpecifitions === 'DIN 8074/8075/2011') {
                                    Array.from(document.getElementById("thivsDiam")).forEach((option) => {
                                        let value = parseFloat(option.value);
                                        
                                        const finalThinkess = `${option.value} * ${(value / 26.17).toFixed(1)}`;
                                        option.innerHTML = finalThinkess;
                                        thinkessGlobal = Number(option.value);   
                                        privateThinkess  = option.innerHTML;
...
document.getElementById('slider').appendChild(exportButton);
            ...
            reader.onload = async function(e) {
                ...
                for (const row of excelData) {
                    ...
                    const data = {
                        ...
                        getbar: row.getDataChangeBar || getDataChangeBar || '',
                        specifications: row.getDataChangeSpecifitions || getDataChangeSpecifitions || '',
...
// وظيفة لإنشاء زرار وإضافته إلى الصفحة
...
// وظيفة لإضافة خيار جديد إلى select
function addOptionToSelect(selectId, optionValue) {
    ...
    if (selectElement) {
        ...
        alert('لم يتم العثور على القائمة المنسدلة.');
    }
}

// وظيفة لإضافة حقل جديد (select) إلى النموذج
function addNewFieldToForm(fieldId, labelText) {
    ...
    if (form) {
        ...
        newLabel.style = 'margin-right: 10px;';
        newSelect.id = fieldId;
        newSelect.style = 'padding: 5px; margin-top: 10px;';
...
// وظيفة لإظهار نافذة منبثقة لإضافة خيار أو حقل
function showPopup() {
    ...
    const input = document.createElement('input');
    input.id = 'popupInput';
    input.placeholder = 'أدخل القيمة هنا';
    input.style = 'margin-bottom: 10px; padding: 5px; width: 100%;';

    const select = document.createElement('select');
    select.id = 'popupSelect';
    select.style = 'margin-bottom: 10px; padding: 5px; width: 100%;';

    const option1 = document.createElement('option');
    option1.value = 'addOption';
    option1.textContent = 'إضافة خيار جديد';

    const option2 = document.createElement('option');
    option2.value = 'addField';
    option2.textContent = 'إضافة حقل جديد';

    select.appendChild(option1);
    select.appendChild(option2);

    const addButton = createButton('addButton', 'إضافة', () => {
        ...
        if (selectedValue === 'addOption') {
            const targetSelectId = prompt('أدخل معرف القائمة المنسدلة لإضافة الخيار إليها:');
            addOptionToSelect(targetSelectId, inputValue);
        } else if (selectedValue === 'addField') {
            addNewFieldToForm(inputValue, `حقل جديد: ${inputValue}`);
        }

        document.body.removeChild(popup);
    });

    const cancelButton = createButton('cancelButton', 'إلغاء', () => {
        document.body.removeChild(popup);
    });
...
// إضافة زر لإظهار النافذة المنبثقة
const addHeightButton = createButton('addHeight', 'إضافة', showPopup);
document.getElementById('grid-Template').appendChild(addHeightButton);